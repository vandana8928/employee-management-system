...
if(isset($_REQUEST['m_reply'])){
	...
	$reply=mysqli_real_escape_string($dbCon,$_POST['m_reply']);
...
<p>&nbsp;</p>
 <?php $m_id=$_GET['id'];
$query1="Select * from `task_reply` join `customers` on `customers`.`user_id`=`task_reply`.`reply_by` where `m_id`='".$m_id."'";
